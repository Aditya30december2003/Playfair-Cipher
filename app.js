...
function encryptByPlayfairCipher(str, key) {
    ...
    return {encryptedText: encrypt(str, kt, ps), keyTable: kt};
}

// Function to calculate encryption/decryption
function calculate() {
    ...
    let result = encryptByPlayfairCipher(inputText, keyword);
    outputText.value = result.encryptedText;
    
    // Display the key table
    let matrixHTML = "<h3>Key Table</h3>";
    matrixHTML += "<table border='1'>";
    for (let i = 0; i < 5; i++) {
        matrixHTML += "<tr>";
        for (let j = 0; j < 5; j++) {
            matrixHTML += "<td>" + result.keyTable[i][j] + "</td>";
        }
        matrixHTML += "</tr>";
    }
    matrixHTML += "</table>";
